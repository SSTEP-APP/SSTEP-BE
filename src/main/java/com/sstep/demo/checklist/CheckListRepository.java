...
public interface CheckListRepository extends JpaRepository<CheckList, Long> {
    ...
    @Query("SELECT ch FROM CheckListManager cm WHERE cm.staff.id = :staffId and cm.checkList.category.id = :categoryId and cm.checkList.date = :date and cm.checkList.isComplete = true ")
    Set<CheckList> findCheckListByStaffIdAndCategoryIdAndIsCompleteAndDate(Long staffId, Long categoryId, String date);

    @Query("SELECT ch FROM CheckListManager cm WHERE cm.staff.id = :staffId and cm.checkList.category.id = :categoryId and cm.checkList.date = :date and cm.checkList.isComplete = false ")
